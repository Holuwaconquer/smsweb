// AI Chatbot for Basedsms Website
...
const websiteKnowledge = {
    services: {
        sms: {
            ...
            description: "Get instant access to temporary phone numbers for SMS verification. Perfect for testing, privacy, or accessing services without using your personal number.",
            steps: [
        ...
        logs: {
            name: "Logs Service",
            description: "Secure access to service credentials and logs for various platforms.",
            steps: [
        ...
        socialBooster: {
            ...
            description: "Grow your social media presence with our boosting services for Instagram, TikTok, and more.",
            steps: [
        ...
        phoneNumbers: {
            name: "Phone Numbers",
            description: "Purchase and use phone numbers from 50+ countries worldwide.",
            steps: [
    ...
    pricing: "We offer flexible pricing plans. Click 'View Pricing' on our homepage to see our competitive rates.",
    contact: "You can reach our support team through the Help Center link in the footer, or use this chat for instant assistance!",
    about: "Basedsms is a fast, secure, and temporary SMS verification service. We provide temporary phone numbers, logs service, social media boosting, and phone numbers from 50+ countries."
};
...
function generateAIResponse(userMessage) {
    ...
    if (message.match(/\b(hi|hello|hey|good morning|good afternoon|good evening)\b/)) {
        return "Hello! 👋 Welcome to Basedsms! I'm your virtual assistant. How can I help you today? You can ask me about our services, pricing, features, or anything else!";
    }
    
    // SMS Verification questions
    if (message.match(/\b(sms|verification|verify|code|message|text)\b/)) {
        return `📱 **SMS Verification Service**\n\n${websiteKnowledge.services.sms.description}\n\n**How it works:**\n${websiteKnowledge.services.sms.steps.map((step, i) => `${i + 1}. ${step}`).join('\n')}\n\nWould you like to get started?`;
    }
    
    // Logs Service questions
    if (message.match(/\b(log|logs|credential|access|account)\b/)) {
        return `📋 **Logs Service**\n\n${websiteKnowledge.services.logs.description}\n\n**How it works:**\n${websiteKnowledge.services.logs.steps.map((step, i) => `${i + 1}. ${step}`).join('\n')}`;
    }
    
    // Social Media Booster questions
    if (message.match(/\b(social|instagram|tiktok|facebook|twitter|follower|like|boost|engagement)\b/)) {
        return `📈 **Social Media Booster**\n\n${websiteKnowledge.services.socialBooster.description}\n\n**How it works:**\n${websiteKnowledge.services.socialBooster.steps.map((step, i) => `${i + 1}. ${step}`).join('\n')}\n\nSupported platforms: Instagram, TikTok, Facebook, Twitter, and more!`;
    }
    
    // Phone Numbers questions
    if (message.match(/\b(phone number|numbers|buy number|purchase|country|countries)\b/)) {
        return `📞 **Phone Numbers Service**\n\n${websiteKnowledge.services.phoneNumbers.description}\n\n**How it works:**\n${websiteKnowledge.services.phoneNumbers.steps.map((step, i) => `${i + 1}. ${step}`).join('\n')}\n\nWe support 50+ countries worldwide!`;
    }
    
    // Pricing questions
    if (message.match(/\b(price|pricing|cost|how much|payment|pay|cheap|expensive|plan)\b/)) {
    ...
    if (message.match(/\b(feature|benefit|why|advantage|what do you offer)\b/)) {
        return `✨ **Our Key Features:**\n\n${websiteKnowledge.features.map(f => `• ${f}`).join('\n')}\n\nWe're committed to providing the best service in the industry!`;
    }
    
    // Sign up / Get started
    if (message.match(/\b(sign up|signup|register|join|create account|get started|start)\b/)) {
    ...
    // Countries / Coverage
    if (message.match(/\b(which countries|what countries|coverage|available|where)\b/)) {
...
class ChatBot {
    ...
    sendWelcomeMessage() {
        setTimeout(() => {
            this.addMessage('bot', "Hi there! 👋 I'm the Basedsms Assistant. I'm here to help you learn about our services. What would you like to know?");
        }, 500);
    }
